...
const STATUS_STYLES: Record<string, string> = {
    Lunas: 'bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-800',
    Sebagian: 'bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-800',
    'Belum Lunas': 'bg-rose-100 text-rose-800 border-rose-300 dark:bg-rose-900/30 dark:text-rose-300 dark:border-rose-800',
};
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="relative z-10 px-6 w-full">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4 text-white mb-2">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight">Nota Supplier Penerimaan Barang</h1>
            ...
            <div className="px-6 w-full -mt-14 relative z-20 pb-12">
                    ...
                    {tabs.map(tab => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-sm border ${
                                activeTab === tab.key
                                    ? 'bg-white dark:bg-neutral-900 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800 shadow-md'
                                    : 'bg-white/70 dark:bg-neutral-900/70 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-neutral-700 hover:bg-white dark:hover:bg-neutral-900'
                            }`}
                        >
                            <tab.icon className="h-4 w-4" />
                            {tab.label}
                            <span className={`ml-1 px-2 py-0.5 rounded-full text-xs ${
                                activeTab === tab.key ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' : 'bg-slate-100 text-slate-500 dark:bg-neutral-800 dark:text-slate-400'
                            }`}>
                ...
                <Card className="shadow-xl border-0 ring-1 ring-black/5 dark:ring-white/5 rounded-2xl overflow-hidden backdrop-blur-sm bg-white/95 dark:bg-neutral-950">
                    <CardHeader className="border-b bg-slate-50/50 dark:bg-neutral-900/50 dark:border-neutral-800">
                        <div className="flex items-center justify-between">
                            ...
                            <TableBody>
                                {filtered.length === 0 ? (
                                    ...
                                    filtered.map(receipt => {
                                        ...
                                        return (
                                            <TableRow key={receipt.id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-900/50 transition-colors">
            ...
            <Dialog open={isAddOpen || isEditOpen} onOpenChange={open => { if (!open) { setIsAddOpen(false); setIsEditOpen(false); } }}>
                <DialogContent className="bg-white dark:bg-neutral-950 border dark:border-neutral-800 shadow-xl sm:max-w-[620px] max-h-[90vh] overflow-y-auto">
                    ...
                    <form onSubmit={handlePaymentSubmit}>
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="bg-slate-50 dark:bg-neutral-900 p-4 rounded-xl border border-slate-200 dark:border-neutral-700">
                                <div className="text-sm text-slate-600 dark:text-slate-300 mb-2 font-medium">Nota yang Dipilih ({selectedReceiptIds.length})</div>
                                <div className="max-h-40 overflow-y-auto space-y-2">
                                    {selectedReceipts.map(r => (
                                        <div key={r.id} className="flex justify-between items-center bg-white dark:bg-neutral-800 p-2 rounded border border-slate-100 dark:border-neutral-700 text-sm">
                                            <div className="font-medium text-slate-800 dark:text-slate-200">{r.nomor_nota} - {r.toko_material?.nama_toko}</div>
                                            <div className="text-emerald-700 font-bold">Sisa: {formatCurrency(r.total_harga - (r.total_paid || 0))}</div>
                                        </div>
                                    ))}
                                </div>
                                <div className="mt-3 pt-3 border-t border-slate-200 dark:border-neutral-700 flex justify-between items-center">
                                    <span className="font-semibold text-slate-700 dark:text-slate-300">Total Sisa Hutang:</span>
                                    <span className="text-xl font-bold text-rose-700 dark:text-rose-400">{formatCurrency(totalSelectedRemaining)}</span>
                                </div>
                            </div>
