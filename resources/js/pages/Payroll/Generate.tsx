    ...
    >
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <p className="text-xs text-gray-500">{checked ? 'Aktif (Disertakan)' : 'Non-Aktif (Rp 0)'}</p>
            </div>
        </div>
...
export default function Generate({ payrollData, period, period_string }: any) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Penggajian', href: route('payroll.index') }, { title: 'Generate', href: '#' }]}>
            ...
            <div className="pb-32">
                <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Proses Hitung Gaji</h1>
                                    ...
                                    <Card key={emp.employee_id} className={`border transition-all duration-300 relative ${!isPaid ? 'opacity-60 grayscale border-dashed border-gray-300 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950/50' : (totalGaji < 0 ? 'border-red-300 dark:border-red-900 bg-red-50 dark:bg-red-950/20' : 'border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950')} shadow-sm hover:shadow-md`}>
                                        ...
                                        <CardContent className="p-6">
                                            ...
                                            <div className="flex justify-between items-start mb-4 border-b border-gray-100 dark:border-zinc-800 pb-4">
                                                ...
                                                {/* Gaji Pokok Menjadi Input */}
                                                <div className="text-right flex flex-col items-end w-1/2 pl-2">
                                                    <Label className="text-xs font-bold text-gray-400 dark:text-zinc-500 uppercase block mb-1">Gaji Pokok</Label>
                                                    <Input
                                                        type="number"
                                                        value={emp.gaji_pokok}
                                                        onChange={(e) => handleInputChange(index, 'gaji_pokok', e.target.value)}
                                                        className={`h-10 w-full text-right font-bold text-sm transition-colors ${data.include_gaji && isPaid ? 'bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-700 focus:ring-emerald-500 text-gray-900 dark:text-white' : 'bg-gray-100 dark:bg-zinc-900 border-transparent text-gray-400 line-through shadow-none'}`}
                                            ...
                                            <div className={`space-y-3 ${!isPaid ? 'pointer-events-none blur-[1px]' : ''}`}>
                                                ...
                                                {/* UANG MAKAN SECTION */}
                                                <div className={`p-4 rounded-xl border transition-colors ${data.include_makan ? 'bg-amber-50/50 border-amber-100 dark:bg-amber-900/10 dark:border-amber-900/30' : 'bg-gray-50 border-gray-100 dark:bg-zinc-900 dark:border-zinc-800'}`}>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <Label className="text-xs text-gray-500 dark:text-zinc-400">Jml Hari</Label>
                                                            <Input
                                                                type="number"
                                                                value={emp.hari_hadir}
                                                                onChange={(e) => handleInputChange(index, 'hari_hadir', e.target.value)}
                                                                className="h-10 text-xs bg-white dark:bg-zinc-950 border-gray-200 dark:border-zinc-800 rounded-xl"
                                                                disabled={!data.include_makan}
                                                            />
                                                        </div>
                                                        <div>
                                                            <Label className="text-xs text-gray-500 dark:text-zinc-400">Tarif /Hari</Label>
                                                            <Input
                                                                type="number"
                                                                value={emp.uang_makan_rate}
                                                                onChange={(e) => handleInputChange(index, 'uang_makan_rate', e.target.value)}
                                                                className="h-10 text-xs bg-white border-amber-200 focus:ring-amber-500 dark:bg-zinc-950 dark:border-amber-900/50 rounded-xl"
                                                                disabled={!data.include_makan}
                                                            />
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <div>
                                                        ...
                                                        <Input type="number" value={emp.insentif} onChange={(e) => handleInputChange(index, 'insentif', e.target.value)} className="h-10 text-sm bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-800 rounded-xl" />
                                                    </div>
                                                    <div className={!data.include_kasbon ? 'opacity-50' : ''}>
                                                        ...
                                                        <Input
                                                            ...
                                                            className="h-10 text-sm border-rose-200 text-rose-600 bg-rose-50/30 focus:ring-rose-500 dark:bg-rose-950/20 dark:border-rose-900/50 dark:text-rose-400 rounded-xl"
                                            ...
                                            {/* Footer Kartu */}
                                            <div className={`flex justify-between items-center p-4 rounded-xl ${isPaid ? 'bg-gray-50 dark:bg-zinc-900' : 'bg-transparent'}`}>
                                                <span className="text-sm font-bold text-gray-600 dark:text-gray-300">Total Terima</span>
                                                <span className={`text-xl md:text-2xl font-black ${!isPaid ? 'text-gray-400' : (totalGaji < 0 ? 'text-rose-600' : 'text-indigo-600 dark:text-indigo-400')}`}>
            ...
            {/* STICKY FOOTER */}
            <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-zinc-950 border-t border-gray-200 dark:border-zinc-800 p-4 md:p-6 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.1)] z-50">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="text-center md:text-left">
                        <p className="text-xs text-gray-500 dark:text-zinc-400 uppercase font-bold tracking-wider mb-1">Total Dana Disiapkan:</p>
                        <p className="text-2xl md:text-3xl font-black text-indigo-600 dark:text-indigo-400">{formatCurrency(grandTotal)}</p>
                    </div>
